...
def input_str(temp=25, pH=8.1, Na=0, Cl=0, K=0, B=0, Ca=0, DIC=0, Mg=0, SO4=0, density=1, pe=4, units='mol/L'):
    """
...
def calc_cb(temp=25, pH=8.1, Na=0, Cl=0, K=0, B=0, Ca=0, DIC=0, Mg=0, SO4=0, density=1, pe=4, units='mol/L', dbase='pitzer', database_path=None, summ=True, phreeq_path='/usr/local/lib/libiphreeqc.so'):  
    """
    ...
    # create input string
    inp = input_str(temp, pH, Na, Cl, K, B, Ca, DIC, Mg, SO4, density, pe, units)
    # run phreeqc
...
def calc_cb_rows(df, dbase='pitzer', phreeq_path='/usr/local/lib/libiphreeqc.so', summ=True):
    """
    ...
    cols = calc_cb(temp=r['Temp (°C)'],
                   ...
                   B=r['[B] (M)'], 
                   DIC=r['[DIC] (M)'], 
                   Mg=r['[Mg] (M)'], dbase=dbase, summ=summ).index
    ...
    for i, r in df.iterrows():
        out.loc[i, :] = calc_cb(temp=r['Temp (°C)'],
                                ...
                                B=r['[B] (M)'], 
                                DIC=r['[DIC] (M)'], 
                                Mg=r['[Mg] (M)'], dbase=dbase, phreeq_path=phreeq_path, summ=summ)
    return out
